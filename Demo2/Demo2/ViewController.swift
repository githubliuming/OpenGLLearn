...
class ViewController: UIViewController {
    var glView:GLKView?;
    var contex:EAGLContext = EAGLContext.init(api: .openGLES2)!;
    var effect:GLKBaseEffect = GLKBaseEffect.init();
    //纹理数组
    ...
    override func viewDidLoad() {
        ...
        let y:CGFloat = (self.view.bounds.size.height - self.view.bounds.size.width)/2.0;
        
        glView = GLKView.init(frame: CGRect.init(x: 0, y: y, width: self.view.bounds.size.width, height: self.view.bounds.size.width ));
...
extension ViewController:GLKViewDelegate{
    func glkView(_ view: GLKView, drawIn rect: CGRect)
    {
        ...
        while index < count {
            ...
            glVertexAttribPointer(GLuint(GLKVertexAttrib.texCoord0.rawValue), 2, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(MemoryLayout<GLfloat>.size * 5), UnsafeRawPointer.init(bitPattern: offset));
